...
class PaieController extends Controller
{
    ...
    // a_des_primes détecté sur les 4 colonnes de prime réelles
    // ──────────────────────────────────────────────────────────────────────────
    public function recapMois(Request $request): JsonResponse
    {
        ...
        $recap = Paie::select(
                'mois',
                DB::raw('COUNT(*) as nb_bulletins'),
                DB::raw('MAX(statut = "modifie") as a_des_primes')
            )
            ->where('annee', $annee)
            ->groupBy('mois')
            ->orderBy('mois')
        ...
        $result = collect(range(1, 12))->map(function ($mois) use ($recap) {
            ...
            return [
                ...
                'a_des_primes' => (bool) ($m?->a_des_primes ?? false),
                'genere'       => $m !== null,
            ];
        });
